...
namespace UnityEngine.UI
{
    ...
    public abstract class LoopScrollRect : UIBehaviour, IInitializePotentialDragHandler, IBeginDragHandler, IEndDragHandler, IDragHandler, IScrollHandler, ICanvasElement, ILayoutElement, ILayoutGroup
    {
        ...
        [SerializeField]
        private float m_Elasticity = 0.1f; // Only used for MovementType.Elastic 弹性
        ...
        [SerializeField]
        private float m_DecelerationRate = 0.135f; // Only used when inertia is enabled 见速率
        ...
        private Vector2 m_PointerStartLocalCursor = Vector2.zero; // 指针起始本地光标
        private Vector2 m_ContentStartPosition = Vector2.zero; // 内容起始位置
        ...
        private Bounds m_ContentBounds; //content包围盒
        private Bounds m_ViewBounds; // view包围盒

        private Vector2 m_Velocity; // 速度
        public Vector2 velocity { get { return m_Velocity; } set { m_Velocity = value; } }

        private bool m_Dragging;

        private Vector2 m_PrevPosition = Vector2.zero; // 上一个位置
        private Bounds m_PrevContentBounds;
        private Bounds m_PrevViewBounds;
        [NonSerialized]
        private bool m_HasRebuiltLayout = false;

        private bool m_HSliderExpand; // 水平slider是否扩张
        private bool m_VSliderExpand; // 竖直slider 是否扩张
        private float m_HSliderHeight; // 水平slider 高
        ...
        public void SrollToCell(int index, float speed)
        {
            ...
            if (speed <= 0)
            {
                RefillCells(index); // speed < 0表示直接重新填充
                return;
            }
        ...
        IEnumerator ScrollToCellCoroutine(int index, float speed)
        {
            ...
            while (needMoving)
            {
                ...
                if (!m_Dragging)
                {
                    ...
                    {
                        m_ViewBounds = new Bounds(viewRect.rect.center, viewRect.rect.size); // viewport or scroll 包围盒
                        ...
                        if (Mathf.Abs(offset) < maxMove)
                        {
                            ...
                            move = Mathf.Sign(offset) * maxMove; // 看不懂
                    }
                    if (move != 0)
                    {
        ...
        protected virtual void LateUpdate()
        {
            ...
            EnsureLayoutHasRebuilt();
            UpdateScrollbarVisibility(); // 更新滑动slider可见性
            UpdateBounds();
        ...
        private void UpdateBounds(bool updateItems = false)
        {
            ...
            // Make sure content bounds are at least as large as view by adding padding if not. 如果不是，则通过添加填充来确保内容边界至少与视图一样大。
            // One might think at first that if the content is smaller than the view, scrolling should be allowed. 一开始可能会认为如果内容比视图小，应该允许滚动。
            // However, that's not how scroll views normally work. 然而，这不是滚动视图通常的工作方式。
            // Scrolling is *only* possible when content is *larger* than view.
            // We use the pivot of the content rect to decide in which directions the content bounds should be expanded. 我们使用内容 react 的枢轴来决定应该向哪个方向扩展内容边界。
            // E.g. if pivot is at top, bounds are expanded downwards. 如果枢轴在顶部，则边界向下扩展。
            // This also works nicely when ContentSizeFitter is used on the content. 当 ContentSizeFitter 用于内容时，这也能很好地工作。
            Vector3 contentSize = m_ContentBounds.size;
            Vector3 contentPos = m_ContentBounds.center;
            Vector3 excess = m_ViewBounds.size - contentSize; //过量
            if (excess.x > 0)
            {
        ...
        private Bounds GetBounds4Item(int index)
        {
            ...
            var toLocal = viewRect.worldToLocalMatrix; //矩阵坐标世界到自身
